...
%Question 2(a)
	...
	%%% Your work goes here! 
		Since $u_1, u_2, u_3$ forms a basis for $W_2$, $\text{dim} W_2 = 3$, and 
		$u_1, u_2, u_3$ all belong to $W_1$, therefore $W_2 \subseteq W_1$. Since 
		$\text{dim} W_1 = \text{dim} W_2 = 3$, therefore $W_1 = W_2$, $W_1 \cap W_2 = W_1$, 
		which is a 3-dimensional subspace of $\mathbb{R}^4$. Therefore $W_1 \cap W_2$ contains a plane.
...
%Question 2(b)
	...
	%%% Your work goes here! 
		$\\$
		Since $w_1, w_2, w_3$ forms a basis for $W_1$, 
		$\text{dim} W_1 = \text{dim span} \{w_1, w_2, w_3\} = 3$, and $w_1, w_2, w_3$ 
		are linearly independent. Since $u_1 \notin W_1$, $u_1$ cannot be expressed as a
		linear combination of $w_1, w_2, w_3$. Therefore $w_1, w_2, w_3, u_1$ are linearly
		independent. Since $w_1, w_2, w_3, u_1 \in \mathbb{R}^4$, are linearly independent, 
